...
export class PgHandler{
    ...
    constructor({ config, querys, allowTransactions = true }) {
        this.config = config;
        this.querys = querys;
    ...
    async beginTransaction(){
        if(!this.allowTransactions){
            throw new Error('Las transacciones no estan permitidas en esta instancia.')
        }
    ...
    async createModel({scriptPath}){
        ...
        try {
            ...
            console.error(`Error al crear el modelo de la base de datos: ${error.message}`);
            throw error;
        } finally {
            conn.release();
        }
